...
export class BattleLogParser {
    ...
    buffer: Buffer
    view: DataView
    data: Map<LogKey, LogValue> = new Map()
    ...
    consumeString() {
        ...
        this.offset += 4
        const result = this.decoder.decode(this.buffer.subarray(this.offset, this.offset + length))
        this.offset += length
        return result
    }
    ...
    parse() {
        this.offset = 0
        this.data.set("header", this.parseHeader())
        this.consumeUint8() // 05 of starting collection
        this.data.set("data", this.parseCollection())
